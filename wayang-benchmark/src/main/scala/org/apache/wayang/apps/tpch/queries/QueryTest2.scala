...
import org.apache.wayang.apps.tpch.data.{Nation, Region}
...
/**
  ...
  * }}}
  */
class QueryTest2(plugins: Plugin*) extends ExperimentDescriptor {
  ...
  def apply(configuration: Configuration,
    ...
    val nation = planBuilder
      ...
      .projectRecords(Seq("n_regionkey", "n_name", "n_nationkey"))
      .withName("Project customers")

      .map(nation => (nation.getLong(0),
            nation.getString(1),
            nation.getLong(2)))
      .withName("Extract regionkey and name and nationkey")
    ...
    nation
      ...
      .map(coli => (coli.field0._2, coli.field1._2, coli.field0._3))
      .withName("Project CO-line-item join product")

      .filter(_._3 > 3, sqlUdf = "n_nationkey > 3", selectivity = .25)
      .withName("Filter nations")

      .map(res => QueryTest2.Result(res._1, res._2))
